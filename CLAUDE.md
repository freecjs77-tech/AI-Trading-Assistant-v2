...
- [SP100 + NDX100 통합 + 섹터 표시](docs/plans/sp100-ndx-merge-with-sectors.md) — SP100_TICKERS 50→~169 (S&P 100 ∪ NASDAQ 100) · GICS 섹터 매핑(Tech/Comm/금융/헬스케어 등 11분류) · 스캐너 표에 Sector 컬럼 추가 · 시그널 로직/캐시키/히스토리 불변 (2026-07-27: 아래 항목으로 169→50 재축소, 섹터 매핑 인프라는 유지)
...
- Momentum Scanner (Step 4c2) + Trade Lifecycle (Step 4c4/4c5) 기능 및 UI 전체 제거 — 관련 모듈/템플릿/테스트/Telegram brief/CI restore 목록 삭제, `fetch_market_data.py`의 EMA9/21/65 lifecycle 전용 필드도 함께 제거. Sister-Repo JSON(`lifecycle_{market}_latest.json`) 소비자(외부 Telegram bot)는 더 이상 데이터를 받지 못함(사용자 확인 완료). gh-pages에 이미 배포된 과거 momentum/lifecycle 파일은 그대로 방치.
- 스캐너 유니버스 재축소 (2026-07-27) — SP100_TICKERS 169→실제 시가총액 상위 50개, KOSPI_TICKERS 101→실제 시가총액 상위 10개로 재선정 (yfinance 실시간 시총 기준; 기존 리스트는 알파벳/업종 그룹 순서라 위치 기준 자르기로는 실제 상위 종목과 불일치했음). `SP100_NAMES`/`TICKER_SECTORS`/`KOSPI_NAMES`/`KOSPI_SECTORS` 동기 축소. Capitol Trades 매칭 유니버스(`politician_trades_fetcher.py`)도 축소된 SP100_TICKERS를 그대로 사용 — 매칭되는 의원 거래 범위가 좁아짐(의도된 부수효과).
- Backtest 페이지 기능 및 UI 전체 제거 — Pipeline Step 6b, `backtest_evaluator.py`(엔진), `rerun_backtest.py`(유지보수 스크립트), `templates/backtest_template.html`, 전 템플릿의 nav 링크 삭제. `history/outcomes.json`·`history/backtest_analysis.json`은 소비자가 없어져 방치(gh-pages 과거 파일도 동일). 수동 CLI 도구인 `historical_backtest.py`/`backtest_1y.py`는 파이프라인/페이지와 무관하여 유지.
